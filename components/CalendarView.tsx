...
const CalendarView: React.FC<CalendarViewProps> = ({ project, onProjectUpdate }) => {
    ...
    const handleTaskDragStart = useCallback((task: Task, type: DragState['type'], e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const handleMouseMove = useCallback((e: MouseEvent) => {
        ...
        if (dragState.type === 'move') {
            ...
            const duration = differenceInDays(dragState.taskInitialEndDate, dragState.taskInitialStartDate);
            newStartDate = addDays(dragState.taskInitialStartDate, totalDayOffset);
    ...
    const handleCreateGroup = () => {
        ...
        onProjectUpdate({ ...project, tasks: updatedTasks, groups: [...project.groups, newGroup] });
...
const MonthGrid: React.FC<MonthGridProps> = ({ month, project, unitColorMap, selectedTaskIds, onDragToCreate, onTaskDragStart, onTaskClick, onDayMouseDown, editingTaskId, editingTaskName, onEditStart, onEditingTaskNameChange, onEditSave, onEditKeyDown }) => {
                    ...
                    return (
                       <div key={weekIndex} className="relative grid grid-cols-7 border-b border-gray-200 task-week-grid">
                            {week.map((day) => (
                                <div 
                                    key={day.toISOString()} 
                                    className={`relative p-1 border-r border-gray-200 ${!isSameMonth(day, month) ? 'bg-gray-50' : 'bg-white'}`}
                                    onMouseDown={(e) => { onDayMouseDown(e); dragStartRef.current = day; }}
                                                ...
                                                >
                                                    {isEditing ? (
                                                        ...
                                                        <>
                                                            ...
                                                            />
                                                            <p className="pointer-events-none whitespace-normal break-words w-full">{task.name}</p>
                                                            <div 
